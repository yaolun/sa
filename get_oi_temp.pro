pro get_oi_temp,filename,outdir,cube=cube,oi=oi,cii=cii
	...
	if not keyword_set(cube) then begin
		readcol,filename,format='A,A,D,D,D,D,D,D,D,D,D,D,D,D,D,D,A,D',$
			object, line_name, lab_wl, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_str, snr, E_u, A, g, ra, dec, blend, valid, skipline=1,/silent
		msg = ''
	endif else begin
		readcol,filename,format='A,A,D,D,D,D,D,D,D,D,D,D,D,D,D,D,A,A,D',$
			object, line_name, lab_wl, cen_wl, sig_cen_wl, str, sig_str, fwhm, sig_fwhm, base_str, snr, E_u, A, g, ra, dec, pix, blend, valid, skipline=1,/silent
		msg = '_cube_'
	endelse
	...
	for i = 0, n_elements(ind)-1 do begin
		...
		if not keyword_set(cube) then begin
			...
			print, format='(a10,2x,a14,2x,g10.8,2x,g14.8,2x,g8.5,2x,g10.4)',pix[j], line_name[j],cen_wl[j],str[j],velo,Tr
			printf, lun, format='(a10,2x,a14,2x,g10.8,2x,g14.8,2x,g8.5,2x,g10.4)',pix[j], line_name[j],cen_wl[j],str[j],velo,Tr
		endelse
	endfor
	free_lun,lun
	close,lun
