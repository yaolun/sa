...
#     pop_dia_1d(o, '/test/cops-spire/', dist['distance'][dist['object'] == o],
#                line_data, species_info, polyfit=False)

# RCrA-IRS7B and 7C HCO+
# select line data
selector1 = (fitting['Object'] == 'RCrA-IRS7C') & (fitting['Pixel_No.'] == 'c') & \
            (fitting['Validity'] == 1) & (fitting['SNR'] >= 4)
selector2 = (fitting['Object'] == 'RCrA-IRS7B') & (fitting['Pixel_No.'] == 'c') & \
            (fitting['Validity'] == 1) & (fitting['SNR'] >= 4)

data_dum = fitting[selector1+selector2]

ind = []
for i in range(len(data_dum)):
    if len(data_dum['Line'][i].split(species_info['name'])[0]) == 0:
        print(data_dum['Line'][i])
        ind.append(i)

line_data = data_dum[ind]

# data_dum = data[ind]
line_data_name = np.array(list(set(data_dum['Line'][ind].data)))

# average the CO data
line_data = {'Line': line_data_name,
           'ObsWL(um)': np.empty(len(line_data_name)),
           'Str(W/cm2)': np.empty(len(line_data_name)),
           'FWHM(um)': np.empty(len(line_data_name)),
           'Noise(W/cm2/um)': np.empty(len(line_data_name)),
           'A(s-1)': np.empty(len(line_data_name)),
           'E_u(K)': np.empty(len(line_data_name)),
           'g': np.empty(len(line_data_name))}
for i, line in enumerate(line_data['Line']):
    line_data['ObsWL(um)'][i] = np.mean(data_dum['ObsWL(um)'][data_dum['Line'] == line])
    line_data['Str(W/cm2)'][i] = np.mean(data_dum['Str(W/cm2)'][data_dum['Line'] == line])
    line_data['FWHM(um)'][i] = np.mean(data_dum['FWHM(um)'][data_dum['Line'] == line])
    line_data['Noise(W/cm2/um)'][i] = np.mean(data_dum['Noise(W/cm2/um)'][data_dum['Line'] == line])
    line_data['A(s-1)'][i] = np.mean(data_dum['A(s-1)'][data_dum['Line'] == line])
    line_data['E_u(K)'][i] = np.mean(data_dum['E_u(K)'][data_dum['Line'] == line])
    line_data['g'][i] = np.mean(data_dum['g'][data_dum['Line'] == line])

print(line_data)
# rotational diagram with fitting
pop_dia_1d('RCrA-IRS7C', '/test/cops-spire/', dist['distance'][dist['object'] == 'RCrA-IRS7C'],
           line_data, species_info, polyfit=False)
